...
WindowsStartup.prototype.functional = function() {
	return ("@mozilla.org/windows-registry-key;1" in Cc);
};
...
WindowsStartup.prototype.saveStartUp = function(enabled, current_user) {
	if (enabled == false) {
		...
		var path = Cc["@mozilla.org/file/directory_service;1"].getService(Ci.nsIProperties).get("CurProcD", Ci.nsIFile);
...
function LinuxStartup(startUp, currentUser) {
	this.configFolder = Cc["@mozilla.org/file/directory_service;1"].getService(Ci.nsIProperties).get("Home", Ci.nsIFile);
